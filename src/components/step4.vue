...
- รวบรวมข้อมูลและสารสนเทศจากกลุ่มผู้รับบริการและผู้มีส่วนได้<br>
ส่วนเสียในรูปแบบต่างๆ เช่น รูปแบบ Big Data เสียง ภาพ และ ข้อความ
- ค้นหาความต้องการ ความคาดหวังของกลุ่มผู้รับบริการ ผู้มีส่วน<br>
  ...
  methods: {
    ...
    getAdvance(data) {
      for (let i = 1; i <= 4; i++) {
        let getData = data.filter(
          x => x.q_number == i.toString() && x.mode == "advance"
        );
    ...
    getIMG(no, mode) {
      ...
      let imgFileName = `${this.$q.sessionStorage.getItem(
        "uid"
      )}-3-${no}-${mode}-${this.$q.sessionStorage.getItem("y")}.jpg`;
