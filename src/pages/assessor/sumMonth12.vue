<template>
  <q-page style="padding:30px 75px;max-width:1400px;margin:auto">
    <div align="center" class="font-36 q-pt-md q-pb-sm relative-position">
      <div class="row">
        <div class="col-10" align="center">รายงานสรุปผลการดำเนินการ 12 เดือน</div>
        <div class="col">
          <q-btn
            @click="changeStatus()"
        ...
        <div style="width:200px;border:3px solid #e84c93;border-radius:5px;">
          <div style="width:100%">
            ...
            <div
              ...
              @click="openFile(1)"
            >
            รายงานสรุปผล<br>การดำเนินการ 12 เดือน
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async getFilePlan() {
      ...
      const url = this.apiPath + "getFile12A_sum.php";
    ...
    async getMonth12(){
      let url = this.apiPath + "getMonth12_sum.php";
      let postData = {
    ...
    async saveData() {
      if (this.suggestion) {
        ...
        let url = this.apiPath + "updateMonth12_a_sum.php";
        let postData = {
          user_id : this.$route.params.userId,
          year : this.$q.sessionStorage.getItem("y"),
    ...
    async deleteFilePlan() {
      let uid = this.$route.params.userId;
      let year = this.$q.sessionStorage.getItem("y");
      const url = this.apiPath + "deleteFile12_a_sum.php";
      let postData = {
        user_id: uid,
        year: year,
        plan: 1,
    ...
    async uploadFile() {
      ...
      const url = this.apiPath + "uploadFile12a_sum.php";
    ...
    async getAssessmentStepperLog(){
      const apiCheckStatus =
        ...
        if(responseCheck.data.length){
          if(responseData.sum_month_12 == '2'){
            this.status = true
          }
        }
