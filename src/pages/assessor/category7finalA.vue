  ...
  margin:auto">
    <div class="row justify-center q-pb-md relative-position">
      ...
      <div class="absolute-right full-height flex flex-center">
        <q-btn
          @click="finishGAP7Final()"
          style="width:160px"
          dense
          class="font-24"
          :color="cat7finalstatus ? 'teal' : null"
          :outline="!cat7finalstatus"
          :label="cat7finalstatus ? 'ประเมินแล้ว' : 'ยังไม่ประเมิน'"
        ></q-btn>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getAssessmentStepperLog() {
      ...
      if (responseData.cat7 == "2") {
        this.cat7finalstatus = true;
      } else {
        this.cat7finalstatus = false;
      }
    },
